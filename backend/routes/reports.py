...
def monthly_report():
    ...
    return ok({
        ...
        "stock_added": {
            "entries": stock.entries or 0,
            "qty": int(stock.qty or 0),
            "value_inr": float(stock.value or 0)
        },
        "allocations": {
            "count": allocs.count or 0,
            "qty": int(allocs.qty or 0)
        },
        "returns": {
            "count": rets.count or 0,
            "qty": int(rets.qty or 0)
        }
    })
...
def employee_report():
    ...
    for emp in employees:
        ...
        total_returned = sum(
            r.qty_returned for a in emp.allocations for r in a.returns
        )
        outstanding_returnable = sum(
        ...
        result.append({
            ...
            "outstanding_returnable": outstanding_returnable,
