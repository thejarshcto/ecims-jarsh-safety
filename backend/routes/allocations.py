...
def allocate():
    ...
    if entry.qty_available < qty:
        return err(f"Insufficient stock. Available: {entry.qty_available}")
    ...
    db.session.commit()
    log_action("ALLOCATE", f"UID: {data['uid']} | Qty: {qty}")
...
def list_allocations():
    ...
    return ok([a.to_dict() for a in query.order_by(Allocation.allocation_date.desc()).all()])
...
def process_return(alloc_id):
    ...
    db.session.add(ret)
    db.session.commit()
    log_action("RETURN", f"Allocation ID: {alloc_id} | Returned: {qty_returned}")
